...
* 지금이 최적의 선택이라고 다음의 선택까지 최적이라는 보장은 절대 없지만, 해당 과제는 이런 간단한 방법으로도 어렵지 않게 풀린다. 우리에게 시간은 많으므로, 원소가 n개일때 스택 전체의 비용을 계산하는 시간복잡도는 O(n)인데, n<=500이므로 충분히 지금 선택과 다음 선택까지 최적인 것도 확인할 수 있을 것 같다. 조금 무리하면 3번의 수행까지도? *O(n^4/8)?* 이러면 명령어 실행 횟수가 유의미하게 줄어들 것 같긴 한데, 이미 통과하기엔 충분하므로 더 이상 시간을 쓰고싶진 않아 그만뒀다. 
